...
def test_create_movement_card_valid(movement_card_service : MovementCardService, session):
    ...
    movement_card = movement_card_service.create_movement_card(mov_type=Movements.DIAGONAL.value, player_owner=1)
    assert session.query(MovementCards).count() == currently + 1
    currently = session.query(MovementCards).count()
    movement_card = movement_card_service.create_movement_card(mov_type=Movements.L.value)
    assert session.query(MovementCards).count() == currently + 1
...
def test_get_all_movement_cards(movement_card_service : MovementCardService, session):
    ...
    movement_card1 = movement_card_service.create_movement_card(mov_type=Movements.L.value, player_owner=1)
    expected = session.query(MovementCards).count()
    assert expected == before + 1
    before = session.query(MovementCards).count() 
    movement_card2 = movement_card_service.create_movement_card(mov_type=Movements.LINE_BETWEEN.value, player_owner=2)
...
def test_get_movement_card_by_id_valid(movement_card_service : MovementCardService, session):
    movement_card1 = movement_card_service.create_movement_card(mov_type=Movements.LINE_BETWEEN.value, player_owner=1)
    assert movement_card_service.get_movement_card_by_id(movement_card_id=1).mov_type == movement_card1.mov_type
    movement_card2 = movement_card_service.create_movement_card(mov_type=Movements.LINE.value, player_owner=2)
...
def test_get_movement_card_by_user_valid(movement_card_service : MovementCardService, session):
    ...
    movement_card2 = movement_card_service.create_movement_card(mov_type=Movements.DIAGONAL.value, player_owner=2)
...
def test_delete_movement_card_from_user_valid(movement_card_service : MovementCardService, session):
    movement_card1 = movement_card_service.create_movement_card(mov_type=Movements.DIAGONAL.value, player_owner=1)
    movement_card2 = movement_card_service.create_movement_card(mov_type=Movements.L.value, player_owner=2)
...
def test_delete_movement_card_valid(movement_card_service, session):
    movement_card1 = movement_card_service.create_movement_card(mov_type=Movements.LINE_BETWEEN.value, player_owner=1)
    movement_card2 = movement_card_service.create_movement_card(mov_type=Movements.INVERSE_DIAGONAL.value, player_owner=2)
    assert session.query(MovementCards).count() == 2
